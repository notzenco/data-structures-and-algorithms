 ...
 * @brief Unit tests for SinglyLinkedList implementation
 */
...
void test_empty_list() {
    ...
    assert(!list.front().has_value());
    assert(!list.back().has_value());
...
void test_insert() {
    ...
    assert(list.get(0).value() == 0);

    assert(list.insert(4, 4));
    assert(list.back().value() == 4);

    assert(!list.insert(100, 99));
...
void test_remove() {
    ...
    assert(list.remove(2).value() == 4);
    assert(list.back().value() == 3);

    assert(!list.remove(100).has_value());
...
void test_copy_constructor() {
    ...
    dsa::SinglyLinkedList<int> list1;
    list1.push_back(1);
    list1.push_back(2);
    list1.push_back(3);

    dsa::SinglyLinkedList<int> list2(list1);
...
void test_move_constructor() {
    ...
    dsa::SinglyLinkedList<int> list2(std::move(list1));

    assert(list2.size() == 2);
    assert(list2.front().value() == 1);
...
void test_string_type() {
    ...
    dsa::SinglyLinkedList<std::string> list;
    list.push_back("hello");
    list.push_back("world");

    assert(list.front().value() == "hello");
    assert(list.back().value() == "world");
    assert(list.find("world").value() == 1);

    std::cout << " PASSED\n";
}

int main() {
    std::cout << "Running SinglyLinkedList tests...\n";
